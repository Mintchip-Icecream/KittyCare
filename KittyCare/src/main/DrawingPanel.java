package main;/*
     ...
     * =====================================================================
     *
     * COMPATIBILITY NOTE: This version of main.DrawingPanel requires Java 8 or higher.
     ...
     * =====================================================================
     *
     * The main.DrawingPanel class provides a simple interface for drawing persistent
     * images using a Graphics object.  An internal BufferedImage object is used
     * to keep track of what has been drawn.  A client of the class simply
     * constructs a main.DrawingPanel of a particular size and then draws on it with
...
import java.util.*;
    ...
    /**
     *
     * main.DrawingPanel is a simplified Java drawing window class to accompany
     ...
     *
     * <p>
     * You can always download the latest {@code main.DrawingPanel} from
     ...
     *
     * <p>
     * The {@code main.DrawingPanel} class provides a simple interface for drawing persistent
     * images using a {@code Graphics} object.  An internal {@code BufferedImage} object is used
     * to keep track of what has been drawn.  A client of the class simply
     * constructs a {@code main.DrawingPanel} of a particular size and then draws on it with
     ...
     * {@code main.DrawingPanel} is not intended to be a full rich graphical library for things
     ...
     *
     * <p>
     * Here is a canonical example of creating a {@code main.DrawingPanel} of a given size and
     * using it to draw a few shapes.
     * </p>
     *
     * <pre>
     * // basic usage example
     * main.DrawingPanel panel = new main.DrawingPanel(600, 400);
     ...
     *
     * <p>
     * This version of {@code main.DrawingPanel} allows you to loop over the pixels of an image.
     ...
     * public static void flipHorizontal(main.DrawingPanel panel) {
     ...
     * events that occur in a {@code main.DrawingPanel} using a lambda function.  For example:
     *
     * <pre>
     ...
     * It includes a {@code showCounts} method for the {@code main.DrawingPanel} itself
    ...
    public class DrawingPanel implements ImageObserver {
        ...
        private static final String ABOUT_MESSAGE       = "main.DrawingPanel\n"
        ...
        private static final String ABOUT_MESSAGE_TITLE = "About main.DrawingPanel";
        ...
        /**
         * The default width of a main.DrawingPanel in pixels, if none is supplied at construction (500 pixels).
         */
        public static final int DEFAULT_WIDTH           = 500;

        /**
         * The default height of a main.DrawingPanel in pixels, if none is supplied at construction (400 pixels).
         */
        ...
        /* a list of all main.DrawingPanel instances ever created; used for saving graphical output */
        ...
        // last main.DrawingPanel is closed
        static {
            try {
        ...
        /*
         * Called when main.DrawingPanel class loads up.
        ...
        private static void checkAnimationSettings() {
            try {
                ...
                if (settingsFile.exists()) {
                    ...
                    input.close();
                    System.out.println("***");
                    System.out.println("*** main.DrawingPanel saving animated GIF: " +
        ...
        /**
         * Returns the file name used for saving all main.DrawingPanel instances.
        ...
        /**
         * Returns true if main.DrawingPanel instances should anti-alias (smooth) their graphics.
        ...
        public static boolean mainIsActive() {
            ...
            for (int i = 0; i < threads.length; i++) {
                ...
                if (DEBUG) System.out.println("    main.DrawingPanel.mainIsActive(): " + thread.getName() + ", priority=" + thread.getPriority() + ", alive=" + thread.isAlive() + ", stack=" + java.util.Arrays.toString(thread.getStackTrace()));
        ...
        /**
         * Saves every main.DrawingPanel instance that is active.
        ...
        /**
         * Sets whether main.DrawingPanel instances should anti-alias (smooth) their pixels by default.
         * Default true.  You can set this on a given main.DrawingPanel instance with setAntialias(boolean).
        ...
        private long createTime;               // time at which main.DrawingPanel was constructed
        ...
        public DrawingPanel(int width, int height) {
            ...
            if (DEBUG) System.out.println("main.DrawingPanel(): going to grab lock");
            synchronized (LOCK) {
                instances++;
                instanceNumber = instances;  // each main.DrawingPanel stores its own int number
                INSTANCES.add(this);

                if (shutdownThread == null && !usingDrJava()) {
                    if (DEBUG) System.out.println("main.DrawingPanel(): starting idle thread");
                        ...
                        public void run() {
                            ...
                            try {
                                while (true) {
                                    ...
                                    save |= shouldSave();
                                    if (DEBUG) System.out.println("main.DrawingPanel idle thread: instances=" + instances + ", save=" + save + ", main active=" + mainIsActive());
                                    if ((instances == 0 || save) && !mainIsActive()) {
                                        try {
                                            System.exit(0);
                                        } catch (SecurityException sex) {
                                            if (DEBUG) System.out.println("main.DrawingPanel idle thread: unable to exit program: " + sex);
                                        }
                                    }
                                ...
                                if (DEBUG) System.out.println("main.DrawingPanel idle thread: exception caught: " + e);
                            }
                        }
                    });
                    // shutdownThread.setPriority(Thread.MIN_PRIORITY);
                    shutdownThread.setName("main.DrawingPanel-shutdown");
            ...
            if (DEBUG) System.out.println("main.DrawingPanel(w=" + width + ",h=" + height + ",anim=" + isAnimated() + ",graph=" + isGraphical() + ",save=" + shouldSave());
            ...
            if (isGraphical()) {
                ...
                try {
                    Runtime.getRuntime().addShutdownHook(new Thread(new Runnable() {
                        ...
                        public void run() {
                            if (DEBUG) System.out.println("main.DrawingPanel.run(): Running shutdown hook");
                            if (DEBUG) System.out.println("main.DrawingPanel shutdown hook: instances=" + instances);
                            try {
                    ...
                    if (DEBUG) System.out.println("main.DrawingPanel(): unable to add shutdown hook: " + e);
                }
            }
        }
        ...
        /*
         * Compares the current main.DrawingPanel image to an image file on disk.
         */
        ...
        /*
         * Compares the current main.DrawingPanel image to an image file on the web.
         */
        ...
        /*
         * Closes the main.DrawingPanel and exits the program.
         */
        private void exit() {
            if (isGraphical()) {
        ...
        private BufferedImage getImage() {
            ...
            // if (DEBUG) System.out.println("main.DrawingPanel getImage setting background to " + backgroundColor);
            g.setColor(backgroundColor);
         ...
         * extracted from this by calling main.DrawingPanel.getRed, getGreen, and getBlue.
         * If nothing has been explicitly drawn on this particular pixel, the panel's
         * background color is returned.
         * See also: getPixel.
         * @param x x-coordinate of pixel to retrieve
        ...
        /**
         * Returns the colors of all pixels in this main.DrawingPanel as a 2-D array
         * of Color objects.
        ...
        */
         ...
         * the main.DrawingPanel is run 'headless' without a GUI, often for scripting
        ...
        public Image loadImage(String filename) {
            ...
            if (!(new File(filename)).exists()) {
                throw new RuntimeException("main.DrawingPanel.loadImage: File not found: " + filename);
            }
...
*/
        ...
        public void save(String filename) throws IOException {
            ...
            try {
                // System.out.println("main.DrawingPanel DEBUG: saving to " + new File(filename).getAbsolutePath());
        ...
        public void saveAnimated(String filename) throws IOException {
            ...
            if (DEBUG) System.out.println("main.DrawingPanel.saveAnimated(" + filename + ")");
        ...
        private void saveAsAnimated() {
            ...
            if (filename != null) {
                try {
                    ...
                    JOptionPane.showMessageDialog(frame,
                            "Due to constraints about how main.DrawingPanel works, you'll need to\n" +
                                    "re-run your program.  When you run it the next time, main.DrawingPanel will \n" +
        ...
        /**
         * Sets the colors of all pixels in this main.DrawingPanel to the colors
         ...
         * If the main.DrawingPanel is not showing on the screen, has no effect.
         ...
         * lambda functions to handle mouse events that occur in a main.DrawingPanel.
         */
        @FunctionalInterface
        ...
        /**
         * This functional interface is provided to allow Java 8 clients to write
         * lambda functions to handle key events that occur in a main.DrawingPanel.
         */
        @FunctionalInterface
        ...
        private class DPWindowListener extends WindowAdapter {
            // called when main.DrawingPanel closes, to potentially exit the program
        ...
        // inner class to do the actual drawing onto the main.DrawingPanel
